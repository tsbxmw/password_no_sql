---
layout: post
title: C 时间矩阵显示 CUBIE TRUCK
categories: C
description: 笔记
keywords: C, LUbuntu, Time, cubie 
---
...
* [C 时间矩阵显示 CUBIE TRUCK](https://tsbxmw.github.io/2016/10/23/C-TimeShow-cubie/)
...
### 起源
  ...
  在cubie truck 上的显示效果并不好，而且打开管道获取命令获取不到，最后只能使用文件方式读取结果了。
...
#### 获取系统时间
  ...
  为了更好的实时同步系统时间并显示，之前采用的 popen 被舍弃了，因为他压根显示不出来，date字段获取时间不完整，只能获取前面的。
  采用system命令获取date，然后写入hello文件，最后读取处理并显示
...
#### 获得时间结果，并输出
  ...
  但是，改的时间短的话会有问题，一直闪烁，可能是性能跟不上。等后面修改了再来填坑。
